...
POSTGRES_DB="${POSTGRES_DB:-mantica}"
POSTGRES_USER="${POSTGRES_USER:-mantica}"
...
if curl -sf "http://localhost:${PORT}/health" > /dev/null 2>&1; then
  ...
  (cd server && go run ./cmd/server) > /tmp/mantica-check-backend.log 2>&1 &
  BACKEND_PID=$!
...
if curl -sf "http://localhost:${FRONTEND_PORT}" > /dev/null 2>&1; then
  ...
  pnpm dev:web > /tmp/mantica-check-frontend.log 2>&1 &
  FRONTEND_PID=$!
