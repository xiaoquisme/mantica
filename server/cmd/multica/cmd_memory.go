...
//   MULTICA_WORKSPACES_ROOT env var > ~/multica_workspaces_<profile> > ~/multica_workspaces
//
// WorkspaceID is resolved from MULTICA_WORKSPACE_ID env var or --workspace-id flag
// (same as other CLI commands).
//
// When running inside a task workdir, agents can always reach the correct DB
// because WorkspaceID is injected by the daemon via MULTICA_WORKSPACE_ID.
...
func openMemoryDB(cmd *cobra.Command) (*execenv.MemoryDB, error) {
	...
	if workspaceID == "" {
		return nil, fmt.Errorf("workspace ID is required: pass --workspace-id or set MULTICA_WORKSPACE_ID")
	}
