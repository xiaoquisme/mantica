...
func newAPIClient(cmd *cobra.Command) (*cli.APIClient, error) {
	...
	if serverURL == "" {
		return nil, fmt.Errorf("server URL not set: use --server-url flag, MANTICA_SERVER_URL env, or 'mantica config set server_url <url>'")
	}

	client := cli.NewAPIClient(serverURL, workspaceID, token)
	// When running inside a daemon task, attribute actions to the agent.
	if agentID := os.Getenv("MANTICA_AGENT_ID"); agentID != "" {
		client.AgentID = agentID
	}
	if taskID := os.Getenv("MANTICA_TASK_ID"); taskID != "" {
...
func resolveServerURL(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "server-url", "MANTICA_SERVER_URL", "")
	if val != "" {
...
func resolveWorkspaceID(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "workspace-id", "MANTICA_WORKSPACE_ID", "")
	if val != "" {
		return val
	}
	profile := resolveProfile(cmd)
...
func requireWorkspaceID(cmd *cobra.Command) (string, error) {
	...
	if id == "" {
		return "", fmt.Errorf("workspace_id is required: use --workspace-id flag, set MANTICA_WORKSPACE_ID env, or run 'mantica config set workspace_id <id>'")
	}
	return id, nil
}
