...
import (
	...
	"github.com/xiaoquisme/mantica/server/internal/daemon"
	logger_pkg "github.com/xiaoquisme/mantica/server/internal/logger"
)
...
func init() {
	...
	f.String("daemon-id", "", "Unique daemon identifier (env: MANTICA_DAEMON_ID)")
	f.String("device-name", "", "Human-readable device name (env: MANTICA_DAEMON_DEVICE_NAME)")
	f.String("runtime-name", "", "Runtime display name (env: MANTICA_AGENT_RUNTIME_NAME)")
	f.Duration("poll-interval", 0, "Task poll interval (env: MANTICA_DAEMON_POLL_INTERVAL)")
	f.Duration("heartbeat-interval", 0, "Heartbeat interval (env: MANTICA_DAEMON_HEARTBEAT_INTERVAL)")
	f.Duration("agent-timeout", 0, "Per-task timeout (env: MANTICA_AGENT_TIMEOUT)")
	f.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: MANTICA_DAEMON_MAX_CONCURRENT_TASKS)")
...
// Empty profile → ~/.mantica/, named profile → ~/.mantica/profiles/<name>/.
...
func runDaemonForeground(cmd *cobra.Command) error {
	...
	serverURL := cli.FlagOrEnv(cmd, "server-url", "MANTICA_SERVER_URL", "")
	if serverURL == "" {
