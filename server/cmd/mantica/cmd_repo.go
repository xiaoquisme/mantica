...
func runRepoCheckout(cmd *cobra.Command, args []string) error {
	...
	daemonPort := os.Getenv("MANTICA_DAEMON_PORT")
	if daemonPort == "" {
		return fmt.Errorf("MANTICA_DAEMON_PORT not set (this command is intended to be run by an agent inside a daemon task)")
	}

	workspaceID := os.Getenv("MANTICA_WORKSPACE_ID")
	agentName := os.Getenv("MANTICA_AGENT_NAME")
	taskID := os.Getenv("MANTICA_TASK_ID")
