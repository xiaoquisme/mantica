...
func resolveToken(cmd *cobra.Command) string {
	if v := strings.TrimSpace(os.Getenv("MANTICA_TOKEN")); v != "" {
		return v
	}
...
func resolveAppURL(cmd *cobra.Command) string {
	for _, key := range []string{"MANTICA_APP_URL", "FRONTEND_ORIGIN"} {
	...
	// self-hosted instances redirect to the correct frontend instead of mantica.ai.
	if err == nil && cfg.ServerURL != "" {
		return strings.TrimRight(cfg.ServerURL, "/")
	}
	return "https://mantica.ai"
}
...
func runAuthStatus(cmd *cobra.Command, _ []string) error {
	...
	if token == "" {
		fmt.Fprintln(os.Stderr, "Not authenticated. Run 'mantica login' to authenticate.")
		return nil
	}
