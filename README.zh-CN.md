...
不再需要复制粘贴 prompt，不再需要盯着运行过程。你的 Agent 出现在看板上、参与对话、随着时间积累可复用的技能。可以理解为开源的 Managed Agents 基础设施——厂商中立、可自部署、专为人类 + AI 团队设计。支持 **Claude Code**、**Codex**、**OpenClaw**、**OpenCode** 和 **Hermes**。
...
- **Agent 流水线** — Issue 流经可配置的多阶段流水线：Classifier → BA → TL → DEV → Code Review → QA。每个阶段由专属 Agent 处理，前一阶段完成后自动推进。
- **可复用技能** — 每个解决方案都成为全团队可复用的技能。部署、数据库迁移、代码审查——技能让团队能力随时间持续增长。
- **统一运行时** — 一个控制台管理所有算力。本地 daemon 和云端运行时，自动检测可用 CLI，实时监控。
- **多工作区** — 按团队组织工作，工作区级别隔离。每个工作区有独立的 Agent、Issue 和设置。
- **定时任务** — 直接在平台上按计划运行周期性 Agent 任务。
- **Web + 桌面端** — 通过 Web 应用或原生 Electron 桌面应用访问 Multica。
...
daemon 会自动检测 PATH 中可用的 Agent CLI（`claude`、`codex`、`openclaw`、`opencode`、`hermes`）。当 Agent 被分配任务时，daemon 会创建隔离环境、运行 Agent、并将结果回传。
...
daemon 在后台运行，保持你的机器与 Multica 的连接。它会自动检测 PATH 中可用的 Agent CLI（`claude`、`codex`、`openclaw`、`opencode`、`hermes`）。
...
进入 **设置 → Agents**，点击 **新建 Agent**。选择你刚连接的 Runtime，选择 Provider（Claude Code、Codex、OpenClaw、OpenCode 或 Hermes），并为 Agent 起个名字——它将以这个名字出现在看板、评论和任务分配中。
...
| 对象存储 | MinIO（本地）或 AWS S3（生产环境） |
| Agent 运行时 | 本地 daemon 执行 Claude Code、Codex、OpenClaw、OpenCode 或 Hermes |

完整架构图请参阅 [`docs/arch.excalidraw`](docs/arch.excalidraw)。

### Agent 流水线

Issue 流经固定的多个阶段。每个 `ready_*` 状态会自动将对应的专属 Agent 分配到该 Issue，Agent 完成后自动推进到下一阶段：

| 状态 | Agent | 职责 |
|------|-------|------|
| `backlog` → `classifying` | Classifier | 分类卡片（技术类 vs 业务类） |
| `ready_analyze` → `in_analyze` | BA | 分解需求 |
| `ready_arch_design` → `in_arch_design` | TL | 输出技术设计方案 |
| `ready_dev` → `in_dev` | DEV | 实现变更 |
| `ready_review` → `in_review` | Code Review | 审查代码差异 |
| `ready_test` → `in_test` | QA | 验证验收标准 |

支持通过扇出实现并行工作流：Agent 可以创建子 Issue 并将父 Issue 设为 `blocked`；当所有子 Issue 达到终态后，平台自动推进父 Issue。
...
```bash
make quickstart     # 一键启动：自动复制 .env.example → .env、安装依赖、启动数据库 + MinIO、执行迁移并运行
```

如需更精细的控制，可分步执行：

```bash
cp .env.example .env   # 先编辑 JWT_SECRET 等密钥
make setup             # 安装依赖、启动数据库 + MinIO、执行迁移
...
```bash
make quickstart     # 一键启动（.env 不存在时自动复制 .env.example）
make setup          # 首次初始化：安装依赖、启动数据库 + MinIO、执行迁移
make start          # 同时启动后端和前端
make stop           # 停止应用进程
